# Usage (when sourced):
#   run_performer "<root_path>" "<data_path>" "<data>"
# Example:
#   run_performer "./data/raw/ETT-small/" "ETTh1.csv" "ETTh1"

run_performer() {
  local root_path="${1:?root_path required}"
  local data_path="${2:?data_path required}"
  local data="${3:?data required}"

  # Prediction horizons to sweep
  local -a PRED_LENS=(24 48 168 336 720)  # ETT exchange_rate weather
  # local -a PRED_LENS=(24 48 168 336)    # ECL traffic
  # local -a PRED_LENS=(24 36 48 60)      # illness

  # Shared args (dataset-specific are injected via the function args above)
  # ETT setup
  # local -a COMMON_ARGS=(
  #   --is_training 1
  #   --model Performer
  #   --root_path "$root_path"
  #   --data_path "$data_path"
  #   --data "$data"
  #   --features M
  #   --target OT
  #   --freq h                
  #   --seq_len 96
  #   --label_len 48
  #   --e_layers 2
  #   --d_layers 1
  #   --factor 5
  #   --num_rand_features 256 
  #   --enc_in 7
  #   --dec_in 7
  #   --c_out 7
  #   --use_torch_compile False
  #   --itr 3
  # )

  # ECL setup
  # local -a COMMON_ARGS=(
  #   --is_training 1
  #   --model Performer
  #   --root_path "$root_path"
  #   --data_path "$data_path"
  #   --data "$data"
  #   --features M
  #   --target OT
  #   --freq h                 
  #   --seq_len 96
  #   --label_len 48
  #   --e_layers 2
  #   --d_layers 1
  #   --factor 5 #
  #   --num_rand_features 256 
  #   --enc_in 321
  #   --dec_in 321
  #   --c_out 321
  #   --use_torch_compile False
  #   --itr 3
  # )

  # # exchange_rate setup
  # local -a COMMON_ARGS=(
  #   --is_training 1
  #   --model Performer
  #   --root_path "$root_path"
  #   --data_path "$data_path"
  #   --data "$data"
  #   --features M
  #   --target OT
  #   --freq h                 
  #   --seq_len 96
  #   --label_len 48
  #   --e_layers 2
  #   --d_layers 1
  #   --factor 3 #
  #   --num_rand_features 256 
  #   --enc_in 8
  #   --dec_in 8
  #   --c_out 8
  #   --use_torch_compile False
  #   --itr 3
  # )

  # # illness
  # local -a COMMON_ARGS=(
  #   --is_training 1
  #   --model Performer
  #   --root_path "$root_path"
  #   --data_path "$data_path"
  #   --data "$data"
  #   --features M
  #   --target OT
  #   --freq h                 
  #   --seq_len 36 
  #   --label_len 18
  #   --e_layers 2
  #   --d_layers 1
  #   --factor 3
  #   --num_rand_features 256 
  #   --enc_in 7
  #   --dec_in 7
  #   --c_out 7
  #   --use_torch_compile False
  #   --itr 3 
  # )

  # # traffic
  # local -a COMMON_ARGS=(
  #   --is_training 1
  #   --model Performer
  #   --root_path "$root_path"
  #   --data_path "$data_path"
  #   --data "$data"
  #   --features M
  #   --target OT
  #   --freq h                 
  #   --seq_len 96
  #   --label_len 48
  #   --e_layers 2
  #   --d_layers 1
  #   --factor 3 #
  #   --num_rand_features 256 
  #   --enc_in 862
  #   --dec_in 862
  #   --c_out 862
  #   --use_torch_compile False
  #   --itr 3 \
  #   --train_epochs 3
  # )

  # weather
  local -a COMMON_ARGS=(
    --is_training 1
    --model Performer
    --root_path "$root_path"
    --data_path "$data_path"
    --data "$data"
    --features M
    --target OT
    --freq h                 
    --seq_len 96
    --label_len 48
    --e_layers 2
    --d_layers 1
    --factor 3 #
    --num_rand_features 256 
    --enc_in 21
    --dec_in 21
    --c_out 21
    --use_torch_compile False
    --itr 3 \
  )

  # MODEL
  for pl in "${PRED_LENS[@]}"; do
    local model_id="Performer_96_${pl}_weather" # manual change
    local des="performer_${pl}"

    local logfile="../logs/performer/${model_id}.log"
    mkdir -p "$(dirname "$logfile")"

    echo "==> Running ${des} (data=weather, pred_len=${pl})" # manual change
    python -u ../run.py \
      "${COMMON_ARGS[@]}" \
      --pred_len "${pl}" \
      --model_id "${model_id}" \
      --des "${des}" \
      | tee "$logfile"
  done
}

# Allow running this file directly as well:
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  if [[ $# -ne 3 ]]; then
    echo "Usage: $0 <root_path> <data_path> <data>" >&2
    exit 64
  fi
  run_performer "$1" "$2" "$3"
fi
